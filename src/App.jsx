...
export default function UniversityQuizApp() {
  ...
  // --- HELPER CALLBACKS (Defined before effects) ---
    ...
    if (screen === 'quiz' && !isPaused && !isBreakActive) {
      timerRef.current = setInterval(() => {
        setTimeRemaining(prev => {
          if (prev <= 1) {
            // Need to wrap finishQuiz to avoid stale closure if it wasn't a callback,
            // but here it's fine as we are calling it inside the setState callback logic 
            // or we should call it outside. 
            // Better pattern for interval completion:
            return 0;
          }
          return prev - 1;
        });
  ...
  useEffect(() => {
    if (timeRemaining === 0 && screen === 'quiz') {
      finishQuiz();
    }
  }, [timeRemaining, screen, finishQuiz]);
...
}, [isBreakActive, breakTimer, endBreak]);
